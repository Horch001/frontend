...
export default function Complaints() {
  ...
  const load = async () => { const r = await adminApi.get('/admin/complaints'); setList(r.data.data.list || []) }
  useEffect(() => { load() }, [])
  const decide = async (id, decision, penaltyPoints = 0) => {
    await adminApi.post(`/admin/complaints/${id}/decide`, { decision, penaltyPoints })
    message.success('已处理')
    load()
  }
  return (
