...
export default function Withdrawals() {
  ...
  const load = async () => { const r = await adminApi.get('/admin/withdrawals'); setList(r.data.data.list) }
  useEffect(() => { load() }, [])
  const review = async (id, action) => { await adminApi.post(`/admin/withdrawals/${id}/review`, { action }); message.success('已处理'); load() }
  return (
    <Card title="提现审核">
