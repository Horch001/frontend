...
export default function ProductsAudit() {
  ...
  const load = async () => { const r = await adminApi.get('/admin/products/pending'); setList(r.data.data.list) }
  useEffect(() => { load() }, [])
  const approve = async (id) => { await adminApi.post(`/admin/products/${id}/approve`); message.success('已通过'); load() }
  return (
    <Card title="待审核商品">
